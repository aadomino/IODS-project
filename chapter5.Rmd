...
The original data, coming from the United Nations Development Programme, is composed of 2 data sets measuring human development and gender inequality for a set of countries. The data used in this part has been modified, however.
...
The data includes 8 variables of human development and inequality and only for those countries that did not have any missing values for these indicators. I start by reading the data and examining its structure and dimensions.

```{r}
human <- read.csv(file = "C:\\Users\\P8Z77-V\\Documents\\GitHub\\IODS-project\\data\\human.csv", row.names = 1)
str(human)
dim(human)
```
